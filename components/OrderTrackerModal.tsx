...
export function OrderTrackerModal() {
  ...
  // Sync active order if it changes
  useEffect(() => {
    if (activeTrackingOrder?.id) {
      setInputQuery(activeTrackingOrder.id);
    }
  }, [activeTrackingOrder]);

  // Close on Escape key & lock body scroll
  useEffect(() => {
    if (!isOrderTrackerOpen) return;
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") setIsOrderTrackerOpen(false);
    };
  ...
  const currentOrder =
    activeTrackingOrder || (placedOrders.length > 0 ? placedOrders[0] : null);
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 sm:p-6 lg:p-8">
      ...
      {/* Modal Card */}
      <div className="relative bg-white dark:bg-zinc-950 text-black dark:text-white rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-2xl max-w-3xl w-full overflow-hidden z-10 my-auto animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="px-7 py-5 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-zinc-50/50 dark:bg-zinc-900/50">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h2 className="text-sm sm:text-base font-bold uppercase tracking-[0.14em]">
                Track Live Dispatch
              </h2>
              <span className="text-[11px] font-mono text-zinc-400">
                AIR CARGO TELEMETRY &amp; STATUS
              </span>
            </div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 divide-y divide-zinc-200 dark:divide-zinc-800">
          {/* Search Bar Input */}
          <div className="p-6 bg-zinc-50 dark:bg-zinc-900/50 space-y-3.5">
            <form onSubmit={handleSearch} className="flex gap-2.5">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full h-12 pl-10 pr-10 rounded-xl bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 text-xs font-mono text-black dark:text-white uppercase placeholder:text-zinc-500 focus:outline-none focus:border-black dark:focus:border-white shadow-xs"
                />
                {inputQuery && (
                  <button
                    type="button"
                    onClick={() => setInputQuery("")}
                    className="absolute right-3.5 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-black dark:hover:text-white p-0.5"
              ...
              <button
                type="submit"
                className="px-6 h-12 rounded-xl bg-black hover:bg-zinc-800 text-white dark:bg-white dark:hover:bg-zinc-200 dark:text-black font-mono font-black text-xs uppercase tracking-[0.14em] transition-all shrink-0 flex items-center gap-2 shadow-sm cursor-pointer active:scale-95"
              >
                <span>Track</span>
          ...
          {currentOrder ? (
            <div className="p-7 space-y-6">
              ...
              <div className="p-5 rounded-2xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 font-mono shadow-xs">
                <div>
              ...
              <div className="space-y-4 font-mono">
                ...
                <div className="space-y-4 relative pl-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-zinc-200 dark:before:bg-zinc-800">
                  {(currentOrder.timeline || [
                    {
                      status: "confirmed",
                      label: "Order Confirmed & Verified",
                    ...
                    {
                      status: "processing",
                      label: "Inspection & Packaging",
                      date: "Scheduled",
                      completed:
                        currentOrder.status === "processing" ||
                        currentOrder.status === "shipped" ||
                        currentOrder.status === "delivered",
                      current: currentOrder.status === "processing",
                    },
                    {
                      status: "in_transit",
                      label: "Air Cargo Dispatch",
                      date: "In Transit",
                      completed:
                        currentOrder.status === "shipped" ||
              ...
              {/* Destination & Carrier Service Grid */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-zinc-200 dark:border-zinc-800 font-mono text-xs">
                <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 space-y-1">
                  ...
                  <p className="font-bold text-black dark:text-white">
                    {currentOrder.customer?.fullName || currentOrder.customerName || "Recipient"}
                  </p>
                ...
                <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 space-y-1">
                  ...
                  <p className="font-bold text-black dark:text-white">
                    {currentOrder.shippingMethod || "BlueDart Express Priority"}
                  </p>
                  <p className="text-zinc-500 dark:text-zinc-400 text-[11px]">
        ...
        <div className="px-7 py-4 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50 flex items-center justify-end">
          <button
            onClick={() => setIsOrderTrackerOpen(false)}
            className="px-6 py-2.5 rounded-xl bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-black dark:text-white font-mono text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer"
          >
            Close Tracker
          </button>
        </div>
